...
from config.aixo import DATABASE_PROVIDER, DATABASE_URL, DATABASE_KEY, DATABASE_SCHEMA
...
def get_client() -> DatabaseClient:
    ...
    if _client is None:
        if DATABASE_PROVIDER == "supabase":
            ...
            _client = SupabaseClient(url=DATABASE_URL, key=DATABASE_KEY, schema=DATABASE_SCHEMA)
        else:
