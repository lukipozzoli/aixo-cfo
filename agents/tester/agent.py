...
from agents.tester.prompt import SYSTEM_PROMPT


class TesterAgent(Agent):
    # Agente de PRUEBA, de solo lectura. Cumple el contrato Agent para que el
    # orquestador lo pueda llamar igual que a cualquier sub-agente. Su caja de
    # herramientas son SOLO tools de lectura inyectadas — no puede escribir.

    name = "tester"
    description = (
        "Agente de solo lectura para pruebas. Consulta cuentas y los ingresos "
        "previstos (cobros a recibir). No modifica nada en la base."
    )
    ...
    def _parse(self, response: str) -> dict | None:
        # Mismo saneo que en financial: limpiar posible envoltorio markdown.
